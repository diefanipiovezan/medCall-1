...
function populaTabelaAgenda(data) {
    ...
    if (data.length === 0) {
        ...
        content += '<thead>';
        content += '<tr>';
        content += '<th class="custom-table-col-index">#</th><th class="custom-table-col-index">Status</th><th>Data</th><th>Dia</th><th>Horário</th><th>Profissional</th><th></th>';
        content += '</tr>';
        content += '</thead>';
        content += '<tbody>';
        $.each(data, function(key, obj) {
            ...
            if (parseInt(obj.status, 10) === 0 || parseInt(obj.status, 10) === 1) {
...
function populaModalDetalhes(data) {
    ...
    content += '<div class="row">';
    content += '<div class="col-md-12">';
    content += '<h4>' + data['dados'][0].profissional + '</h4>';
    content += '</div>';
    content += '</div>';

    content += '<div class="row">';
    content += '<div class="col-md-4">';
    content += 'Registro: ' + data['dados'][0].registro;
    content += '</div>';
...
function alterarAgendamentoPaciente(idAgendamento) {
    $.ajax({
        type: 'POST',
        url: '/agenda/paciente/cancel',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data: { idAgendamento: idAgendamento },
        success: function(data) {
            buscaAgendaPaciente();
...
$(document).ready(function() {
    ...
    $('#tblAngedaPaciente').on('click', '.btn-agendamento-info', function() {
        var btn = $(this);
    ...
    $('#tblAngedaPaciente').on('click', '.btn-agendamento-cancelar', function() {
        var btn = $(this);
        var idAgendamento = btn.data('id_agendamento');
        ...
        btnModalSubmit.removeClass('btn-default, btn-primary');
    ...
    $('#btn-modal-alerta-submit').on('click', function() {
        ...
        alterarAgendamentoPaciente(idAgendamento);
    });
